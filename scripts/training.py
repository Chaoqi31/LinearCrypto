...
def set_global_seed(seed: int):
    """
    Force deterministic behavior across Python, NumPy, and PyTorch.
    """
    random.seed(seed)
    np.random.seed(seed)
...
if __name__ == "__main__":
    ...
    if args.save_checkpoints:
        # Callback 1: Save best checkpoint based on validation RMSE
        ...
        # Callback 2: Save last epoch checkpoint with epoch number in filename
        last_epoch_callback = pl.callbacks.ModelCheckpoint(
            save_top_k=1,
            verbose=True,
            monitor="epoch",
            mode="max",
        ...
        callbacks.append(last_epoch_callback)
